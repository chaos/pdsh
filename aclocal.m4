...
dnl
dnl @version $Id: acx_pthread.m4 398 2003-05-21 17:22:31Z grondo $
...
[
  ...
  #
  SDRPATH="/usr/lpp/ssp/bin:/usr/sbin:$PATH"
  AC_PATH_PROG([SDRGETOBJECTS], [SDRGetObjects], [], [$SDRPATH])
  ...
  if test "$ac_with_qshell" = yes; then
     ...
     if test "$ac_with_pam" = "yes" ; then
        ...
        if test "$ac_have_pam" != "yes" ; then
		   ...
		   AC_MSG_ERROR([Error configuring qshell])
        fi
     else
        ac_have_pam=yes         
     fi
